...
//        self.socket = WebSocket.init(url: URL.init(string: wsUri)!)
        self.socket = WebSocket.init(url: URL.init(string: wsUri)!,protocols: ["protoo"])
...
//        self.socket!.disableSSLCertValidation = true
    ...
    func sendWithAck(message: JSON, completionHandler: @escaping (_: JSON) -> Void) {
        let event: String = message["id"].stringValue
    ...
    func websocketDidReceiveMessage(socket: WebSocketClient, text: String) {
        ...
        var event: String = json["method"].stringValue
        if json["response"].bool == true{
            event = json["id"].stringValue
        }
